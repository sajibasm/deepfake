...
# Check if MPS (Mac GPU) or ROCm (AMD GPU) is available
...
TEST_DIR = "data/test"
MODEL_DIR = "model"
MODEL_PATH = os.path.join(MODEL_DIR, "deepfake_xception.pth")
...
LEARNING_RATE = 0.001
MIN_DELTA = 0.001
EARLY_STOP_PATH = os.path.join(MODEL_DIR, "best_xception_model.pth")
...
print("🚀 Training Xception with Early Stopping...")
...
plt.savefig(os.path.join(MODEL_DIR, "xception_roc_curve.png"))
plt.show()
...
plt.legend(loc="lower right")
plt.savefig(os.path.join(MODEL_DIR, "xception_pr_curve.png"))
plt.show()
...
plt.savefig(os.path.join(MODEL_DIR, "xception_confusion_matrix.png"))
plt.show()
